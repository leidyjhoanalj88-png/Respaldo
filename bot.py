    ...
    try:
        ...
        if "26" in datos:
            ...
            while sub_pos + 4 <= len(sub):
                ...
                try: s_len = int(sub[sub_pos+2:sub_pos+4])
                except ValueError: break
                s_val = sub[sub_pos+4:sub_pos+4+s_len]
                if s_tag == "02" and s_val.strip(): return s_val.strip()
                sub_pos += 4 + s_len
...
def extraer_nombre_qr(contenido):
    ...
    if nombre_emv: return nombre_emv
    if "name=" in contenido.lower() or "&" in contenido or "=" in contenido:
        try:
            ...
            for key in ["name", "Name", "merchant", "businessName", "alias", "comercio", "negocio"]:
                if key in params and params[key].strip(): return params[key].strip()
        except Exception: pass
...
def agregar_vencimiento(user_id, nombre, dias=30):
    ...
    vencimientos[str(user_id)] = {"nombre": nombre, "fecha_vence": fecha_vence, "dias": dias, "aviso3_enviado": False, "expirado_enviado": False}
...
def eliminar_vencimiento(user_id):
    ...
    if str(user_id) in vencimientos:
        del vencimientos[str(user_id)]; guardar_vencimientos(vencimientos)

async def verificar_vencimientos(context: ContextTypes.DEFAULT_TYPE):
    vencimientos = cargar_vencimientos(); hoy = date.today(); actualizar = False
    for uid_str, info in vencimientos.items():
        uid = int(uid_str); fecha_vence = date.fromisoformat(info["fecha_vence"])
        dias_restantes = (fecha_vence - hoy).days
        if dias_restantes == 3 and not info.get("aviso3_enviado"):
            try:
                await context.bot.send_message(chat_id=uid, text=f"⚠️ *Aviso*\n\nHola {info['nombre']}, tu acceso vence en *3 días* ({fecha_vence.strftime('%d/%m/%Y')}).\n\nRenueva con un admin 👑 @nequixxx", parse_mode="Markdown")
                info["aviso3_enviado"] = True; actualizar = True
            except Exception as e: logging.error(f"[VENC] {uid}: {e}")
        elif dias_restantes <= 0 and not info.get("expirado_enviado"):
            try:
                await context.bot.send_message(chat_id=uid, text=f"🔴 *Acceso Expirado*\n\nHola {info['nombre']}, tu acceso expiró.\nContacta al admin:", parse_mode="Markdown",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("👑 ADM", url="tg://user?id=7422843477")], [InlineKeyboardButton("📢 Grupo", url=GROUP_LINK)]]))
                auth_system.remove_user(uid); info["expirado_enviado"] = True; actualizar = True
            except Exception as e: logging.error(f"[VENC] {uid}: {e}")
    if actualizar: guardar_vencimientos(vencimientos)
...
async def generar_y_enviar_a_chat(chat_id, context, fn, data, config, caption=" "):
    ...
    try:
        ...
        logging.error(f"[ERROR] {tb}")
        await context.bot.send_message(chat_id=chat_id, text=f"❌ Error generando:\n<code>{tb[-600:]}</code>", parse_mode="HTML")
        return False
    finally:
...
def construir_resumen(data, tipo):
    tipo_nombres = {
        "comprobante1": "🟣 Nequi", "comprobante4": "💸 Transfiya",
        "comprobante_qr": "📱 Nequi QR", "comprobante_nuevo": "🔑 Bre B",
        "comprobante_anulado": "❌ Anulado", "comprobante_ahorros": "🏦 Ahorros BC",
        "comprobante_corriente": "🏦 Corriente BC", "comprobante_daviplata": "🔴 Daviplata",
        "comprobante_bc_nq_t": "🏦➡️🟣 BC a NQ", "comprobante_bc_qr": "🏦 BC QR",
        "comprobante_nequi_bc": "🟣➡️🏦 Nequi Corriente", "comprobante_nequi_ahorros": "🟣➡️🏦 Nequi Ahorros",
    }
    v = data.get("valor", 0)
    lineas = ["📋 *Confirma los datos:*", "━━━━━━━━━━━━━━━━━", f"📌 {tipo_nombres.get(tipo, tipo)}"]
...
async def mostrar_confirmacion(update, user_id, data, tipo):
    ...
    await update.message.reply_text(construir_resumen(data, tipo), parse_mode="Markdown", reply_markup=confirm_keyboard(user_id))


async def ejecutar_generacion(chat_id, context, user_id):
    if user_id not in user_data_store:
        await context.bot.send_message(chat_id=chat_id, text="❌ Sesión expirada. Usa /comprobante"); return
    data = user_data_store[user_id]; tipo = data["tipo"]; v = data.get("valor", 0)
    ...
    if ok:
        await context.bot.send_message(chat_id=chat_id, text="✅ *Comprobante generado con éxito*\n\nUsa /comprobante para generar otro", parse_mode='Markdown')
...
async def confirmar_generacion_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    action = parts[1]; user_id = int(parts[2])
    if query.from_user.id != user_id:
        await query.answer("❌ Esta confirmación no es tuya.", show_alert=True); return
    if action == "no":
        limpiar_usuario(user_id)
        await query.edit_message_text("❌ Cancelado.\n\nUsa /comprobante para empezar de nuevo."); return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type in ("group", "supergroup") and chat_id != ALLOWED_GROUP: return
    ...
    if not auth_system.is_admin(user_id):
        if not auth_system.can_use_bot(user_id, chat_id) and not auth_system.gratis_mode:
            await update.message.reply_text("🔴 *Bot en Modo OFF*\n\n💰 Contacta a un admin.", parse_mode='Markdown', reply_markup=admin_keyboard()); return
    keyboard = [
    ...
    await update.message.reply_text("Selecciona el comprobante:", reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=False))

async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id; chat_id = update.effective_chat.id
    if auth_system.is_banned(user_id): return
    if not auth_system.gratis_mode and not auth_system.can_use_bot(user_id, chat_id):
        await update.message.reply_text("⛔ Sin acceso.", reply_markup=admin_keyboard()); return
    ...
    try:
        ...
        nparr = np.frombuffer(file_bytes, np.uint8); img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
        detector = cv2.QRCodeDetector(); contenido, _, _ = detector.detectAndDecode(img)
        if not contenido: await update.message.reply_text("❌ No se pudo leer el QR."); return
    ...
    except ImportError: await update.message.reply_text("❌ Error interno.")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in button_mapping:
        if not auth_system.is_admin(user_id) and not auth_system.can_use_bot(user_id, chat_id) and not auth_system.gratis_mode:
            await update.message.reply_text("🔴 Bot en Modo OFF", reply_markup=admin_keyboard()); return
        limpiar_usuario(user_id)
        tipo = button_mapping[text]; user_data_store[user_id] = {"step": 0, "tipo": tipo}
        prompts = {
        ...
        await update.message.reply_text(prompts.get(tipo, "Ingresa los datos:")); return

    if user_id in user_data_store and user_data_store[user_id].get("step") == "qr_monto":
        data = user_data_store[user_id]
        limpio = limpiar_valor(text)
        if not limpio.replace("-","",1).isdigit(): await update.message.reply_text("❌ Valor numérico."); return
        valor = int(limpio)
        if valor < 1000: await update.message.reply_text("❌ Mínimo $1,000."); return
        data["valor"] = valor
        await mostrar_confirmacion(update, user_id, data, data["tipo"]); return

    if user_id not in user_data_store: return
    data = user_data_store[user_id]; tipo = data["tipo"]; step = data["step"]

    if data.get("_pendiente"):
        await update.message.reply_text("⏳ Esperando tu confirmación arriba 👆"); return

    # ── NEQUI ─────────────────────────────────────────────────────────────
    if tipo == "comprobante1":
        if step == 0: data["nombre"] = text; data["step"] = 1; await update.message.reply_text("📱 Número de teléfono")
        elif step == 1:
            ...
            if not tel.isdigit() or len(tel)!=10 or not tel.startswith("3"):
                await update.message.reply_text("❌ 10 dígitos, empieza en 3.\nEjemplo: 3001234567\n\n/cancelar para reiniciar"); return
            data["telefono"] = tel; data["step"] = 2; await update.message.reply_text("💰 Valor")
        elif step == 2:
            limpio = limpiar_valor(text)
            if not limpio.replace("-","",1).isdigit(): await update.message.reply_text("❌ Solo números"); return
            v = int(limpio)
        ...
        elif step == 3: data["fecha_manual"]=text; await mostrar_confirmacion(update, user_id, data, tipo)
        elif step == 10:
            data["referencia_manual"]=text
            if fecha_manual_mode.get(user_id): data["step"]=11; await update.message.reply_text("📅 Fecha:"); return
            await mostrar_confirmacion(update, user_id, data, tipo)
        elif step == 11: data["fecha_manual"]=text; await mostrar_confirmacion(update, user_id, data, tipo)

    # ── TRANSFIYA ─────────────────────────────────────────────────────────
    elif tipo == "comprobante4":
        if step == 0:
            if not text.isdigit() or len(text)!=10 or not text.startswith('3'):
                await update.message.reply_text("❌ 10 dígitos, empieza en 3. /cancelar"); return
            data["telefono"]=text; data["step"]=1; await update.message.reply_text("💰 Valor")
        elif step == 1:
            limpio = limpiar_valor(text)
            if not limpio.replace("-","",1).isdigit(): await update.message.reply_text("❌ Solo números"); return
            v = int(limpio)
            ...
            await mostrar_confirmacion(update, user_id, data, tipo)
        elif step == 2: data["fecha_manual"]=text; await mostrar_confirmacion(update, user_id, data, tipo)
        elif step == 10:
            data["referencia_manual"]=text
            if fecha_manual_mode.get(user_id): data["step"]=11; await update.message.reply_text("📅 Fecha:"); return
            await mostrar_confirmacion(update, user_id, data, tipo)
    ...
    # ── NEQUI QR ──────────────────────────────────────────────────────────
        ...
        if step == 0: data["nombre"]=text; data["step"]=1; await update.message.reply_text("💰 Valor")
        elif step == 1:
            limpio = limpiar_valor(text)
            if not limpio.replace("-","",1).isdigit(): await update.message.reply_text("❌ Solo números"); return
            v = int(limpio)
    ...
    # ── AHORROS ───────────────────────────────────────────────────────────
        ...
        if step == 0: data["nombre"]=text; data["step"]=1; await update.message.reply_text("🏦 Número de cuenta")
        elif step == 1:
            ...
            data["numero_cuenta"]=text; data["step"]=2; await update.message.reply_text("💰 Valor")
        elif step == 2:
            limpio = limpiar_valor(text)
            if not limpio.replace("-","",1).isdigit(): await update.message.reply_text("❌ Solo números"); return
            v = int(limpio)
    ...
    # ── DAVIPLATA ─────────────────────────────────────────────────────────
        ...
        if step == 0: data["nombre"]=text; data["step"]=1; await update.message.reply_text("💰 Valor")
            ...
            v = int(limpio)
            if v < 1000: await update.message.reply_text("Mínimo $1,000"); return
            data["valor"]=v; data["step"]=2; await update.message.reply_text("📤 4 dígitos cuenta que ENVÍA")
        elif step == 2:
            if not text.isdigit() or len(text)!=4: await update.message.reply_text("4 dígitos exactos"); return
            data["recibe"]=text; data["step"]=3; await update.message.reply_text("📥 4 dígitos cuenta que RECIBE")
        elif step == 3:
            if not text.isdigit() or len(text)!=4: await update.message.reply_text("4 dígitos exactos"); return
            data["envia"]=text
            if fecha_manual_mode.get(user_id): data["step"]=4; await update.message.reply_text("📅 Fecha:"); return
            await mostrar_confirmacion(update, user_id, data, tipo)
        elif step == 4: data["fecha_manual"]=text; await mostrar_confirmacion(update, user_id, data, tipo)
    ...
    # ── BC QR ─────────────────────────────────────────────────────────────
        ...
        if step == 0: data["descripcion_qr"]=text; data["step"]=1; await update.message.reply_text("💰 Valor")
        elif step == 1:
            limpio = limpiar_valor(text)
            if not limpio.replace("-","",1).isdigit(): await update.message.reply_text("❌ Solo números"); return
            v = int(limpio)
            if v < 1000: await update.message.reply_text("Mínimo $1,000"); return
            data["valor"]=v; data["step"]=2; await update.message.reply_text("👤 Nombre")
        elif step == 2: data["nombre"]=text; data["step"]=3; await update.message.reply_text("🏦 Número de cuenta")
        elif step == 3:
            digitos = "".join(ch for ch in text if ch.isdigit())
            if len(digitos)<8: await update.message.reply_text("❌ Número de cuenta inválido"); return
    ...
    # ── NEQUI CORRIENTE ───────────────────────────────────────────────────
        ...
        if step == 0: data["nombre"]=text; data["step"]=1; await update.message.reply_text("💰 Valor")
            ...
            v = int(limpio)
            if v < 1000: await update.message.reply_text("Mínimo $1,000"); return
            data["valor"]=v; data["step"]=2; await update.message.reply_text("🏦 Número de cuenta")
        elif step == 2:
            digitos = "".join(ch for ch in text if ch.isdigit())
            if len(digitos)!=11: await update.message.reply_text("Cuenta debe tener 11 dígitos"); return
            data["numero_cuenta"]=text
    ...
    # ── BRE B ─────────────────────────────────────────────────────────────
        ...
        if step == 0: data["nombre"]=text; data["step"]=1; await update.message.reply_text("💰 Valor")
            ...
            try:
                limpio = limpiar_valor(text); v = float(limpio)
                if v < 1000: await update.message.reply_text("Mínimo $1,000"); return
                data["valor"]=v
            except: await update.message.reply_text("❌ Solo números"); return
            data["step"]=2; await update.message.reply_text("🔑 Llave")
        elif step == 2: data["llave"]=text; data["step"]=3; await update.message.reply_text("🏛️ Banco")
        elif step == 3: data["banco"]=text; data["step"]=4; await update.message.reply_text("📞 Número de quien envía")
        elif step == 4:
            if not text.isdigit() or len(text)!=10 or not text.startswith('3'):
                await update.message.reply_text("❌ 10 dígitos, empieza en 3. /cancelar"); return
            ...
            await mostrar_confirmacion(update, user_id, data, tipo)
        elif step == 5: data["fecha_manual"]=text; await mostrar_confirmacion(update, user_id, data, tipo)
        elif step == 10:
            data["referencia_manual"]=text
            if fecha_manual_mode.get(user_id): data["step"]=11; await update.message.reply_text("📅 Fecha:"); return
            await mostrar_confirmacion(update, user_id, data, tipo)
    ...
    # ── AGREGAR USUARIO ───────────────────────────────────────────────────
        ...
        if step == 0:
            if not text.strip().isdigit(): await update.message.reply_text("❌ ID numérico.\nEjemplo: 7422843477\n\n/cancelar"); return
            data["target_user_id"]=int(text.strip()); data["step"]=1; await update.message.reply_text("📝 Nombre del usuario:")
        elif step == 1: data["nombre"]=text; data["step"]=2; await update.message.reply_text("📅 ¿Cuántos días de acceso? (Ejemplo: 30)")
        elif step == 2:
            ...
            try:
                ...
                try:
                    await context.bot.send_message(chat_id=data["target_user_id"],
                        text=f"✅ *¡Acceso Activado!*\n\nHola {data['nombre']}!\n📅 Vence: *{fecha_vence}*\n⏳ Duración: *{dias} días*\n\nUsa /comprobante para empezar.",
                        parse_mode="Markdown")
                except Exception as e: logging.error(f"No se notificó a {data['target_user_id']}: {e}")
                if user_id != ADMIN_ID:
                    await context.bot.send_message(chat_id=ADMIN_ID,
                        text=f"🔔 *Nuevo usuario*\nID: `{data['target_user_id']}`\nNombre: {data['nombre']}\nVence: {fecha_vence}",
                        parse_mode='Markdown')
                limpiar_usuario(user_id)
            except Exception as e:
                await update.message.reply_text(f"❌ Error: {e}"); limpiar_usuario(user_id)
...
async def gratis_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not auth_system.is_admin(update.effective_user.id): await update.message.reply_text("❌ Solo admins."); return
    auth_system.set_gratis_mode(True); await update.message.reply_text("✅ Modo GRATIS activado.")

async def off_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not auth_system.is_admin(update.effective_user.id): await update.message.reply_text("❌ Solo admins."); return
    auth_system.set_gratis_mode(False); await update.message.reply_text("🔴 Modo OFF activado.")
...
async def ban_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try: auth_system.ban_user(int(context.args[0])); await update.message.reply_text(f"🚫 {context.args[0]} baneado.")
    except ValueError: await update.message.reply_text("❌ ID inválido.")

async def unban_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not auth_system.is_admin(update.effective_user.id): await update.message.reply_text("❌ Solo admins."); return
    if not context.args: await update.message.reply_text("Uso: /unban <id>"); return
    try: auth_system.unban_user(int(context.args[0])); await update.message.reply_text(f"✅ {context.args[0]} desbaneado.")
    except ValueError: await update.message.reply_text("❌ ID inválido.")

async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not auth_system.is_admin(update.effective_user.id): await update.message.reply_text("❌ Solo admins."); return
    s = auth_system.get_stats()
    await update.message.reply_text(f"📊 *Stats*\n\n👥 Autorizados: {s['total_authorized']}\n🚫 Baneados: {s['total_banned']}\n🛡️ Admins: {s['total_admins']}\n🆓 Gratis: {'Sí' if s['gratis_mode'] else 'No'}", parse_mode='Markdown')

async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not auth_system.is_admin(update.effective_user.id): await update.message.reply_text("❌ Solo admins."); return
    if not context.args: await update.message.reply_text("Uso: /admin <id>"); return
    try: auth_system.add_admin(int(context.args[0])); await update.message.reply_text(f"✅ {context.args[0]} es admin.")
    except ValueError: await update.message.reply_text("❌ ID inválido.")
...
async def unadmin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try: auth_system.remove_admin(int(context.args[0])); await update.message.reply_text(f"✅ {context.args[0]} ya no es admin.")
    except ValueError: await update.message.reply_text("❌ ID inválido.")

async def cancelar_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if user_id in user_data_store: limpiar_usuario(user_id); await update.message.reply_text("✅ Operación cancelada.")
    else: await update.message.reply_text("No tienes acciones activas. Usa /comprobante.")

async def fechas_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if auth_system.is_banned(user_id): return
    if fecha_manual_mode.get(user_id): fecha_manual_mode[user_id]=False; await update.message.reply_text("📅 Fecha *Automática*.", parse_mode='Markdown')
    else: fecha_manual_mode[user_id]=True; await update.message.reply_text("📅 Fecha *Manual* activado.", parse_mode='Markdown')

async def refes_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if auth_system.is_banned(user_id): return
    if referencia_manual_mode.get(user_id): referencia_manual_mode[user_id]=False; await update.message.reply_text("🔢 Referencia *Automática*.", parse_mode='Markdown')
    else: referencia_manual_mode[user_id]=True; await update.message.reply_text("🔢 Referencia *Manual* activado.", parse_mode='Markdown')

async def precios_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("💵 *PRECIOS*\n\n• 1 Mes: $25,000\n• 2 Meses: $45,000\n• 3 Meses: $55,000\n\n📞 Contacta:", parse_mode='Markdown', reply_markup=admin_keyboard())

async def horarios_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🕰️ *HORARIOS GRATIS*\n\n🌅 9:00 AM - 11:00 AM\n🌞 12:00 PM - 3:00 PM\n\n👑 VIP: 24/7\n\n💎 /precios", parse_mode='Markdown')

async def panel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if not auth_system.is_main_admin(user_id): await update.message.reply_text("⛔ Solo ADM 1."); return
    s = auth_system.get_stats()
    texto = (f"👑 *PANEL ADM 1*\n━━━━━━━━━━━━━━━━━\n\n📊 Modo: {'🟢 GRATIS' if s['gratis_mode'] else '🔴 OFF'}\n"
             f"👥 Autorizados: {s['total_authorized']}\n🚫 Baneados: {s['total_banned']}\n🛡️ Admins: {s['total_admins']}\n\n"
             "━━━━━━━━━━━━━━━━━\n⚙️ *COMANDOS:*\n\n"
             "🆓 /gratis | 🔒 /off\n➕ /agregar | ➖ /eliminar [ID]\n"
             "🚫 /ban [ID] | ✅ /unban [ID]\n🛡️ /admin [ID] | ❌ /unadmin [ID]\n📊 /stats\n━━━━━━━━━━━━━━━━━")
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("🟢 GRATIS", callback_data="panel_gratis"), InlineKeyboardButton("🔴 OFF", callback_data="panel_off")], [InlineKeyboardButton("📊 Stats", callback_data="panel_stats")]])
    await update.message.reply_text(texto, parse_mode='Markdown', reply_markup=kb)

async def panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not auth_system.is_main_admin(query.from_user.id): await query.answer("⛔ Sin permisos.", show_alert=True); return
    await query.answer()
    if query.data == "panel_gratis": auth_system.set_gratis_mode(True); await query.edit_message_text("✅ Modo GRATIS activado.")
    elif query.data == "panel_off": auth_system.set_gratis_mode(False); await query.edit_message_text("🔒 Modo OFF activado.")
    elif query.data == "panel_stats":
        s = auth_system.get_stats()
        await query.edit_message_text(f"📊 *Stats*\nModo: {'🟢' if s['gratis_mode'] else '🔴'}\nAutorizados: {s['total_authorized']}\nBaneados: {s['total_banned']}", parse_mode='Markdown')

async def apk_precios_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    await query.message.reply_text("💵 *PRECIOS*\n\n• 1 Mes: $25,000\n• 2 Meses: $45,000\n• 3 Meses: $55,000\n\n📞 Contacta:", parse_mode='Markdown', reply_markup=admin_keyboard())
...
async def refe_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        photo = update.message.reply_to_message.photo[-1]; referencias = cargar_referencias()
        nueva = {"file_id": photo.file_id, "guardado_por": update.effective_user.first_name or "Admin",
                 "user_id": user_id, "fecha": datetime.now(pytz.timezone("America/Bogota")).strftime("%d/%m/%Y %H:%M:%S"), "numero": len(referencias)+1}
        referencias.append(nueva); guardar_referencias(referencias)
...
async def enviar_referencias_paginadas(update_or_query, context, referencias, offset):
    ...
    total = len(referencias); fin = min(offset+5, total); message_ids = []
    for ref in referencias[offset:fin]:
        caption = f"📸 *#{ref['numero']}* — {ref['guardado_por']} — {ref['fecha']}"
        try:
            file = await context.bot.get_file(ref['file_id']); file_path = await file.download_to_drive()
            ...
            message_ids.append(msg.message_id)
    if fin < total:
        kb = InlineKeyboardMarkup([[InlineKeyboardButton(f"📥 Ver más ({fin+1}-{min(fin+5,total)} de {total})", callback_data=f"ref_next_{fin}_{','.join(map(str,message_ids))}")]])
...
async def referencias_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    parts = query.data.split('_'); offset = int(parts[2])
